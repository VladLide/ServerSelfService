...
public class UsersAccess {
	...
	public static UsersAccess get(int uId, int aId, MySQL db) {
	...
	public static ObservableList<UsersAccess> getList(int uId, int aId, MySQL db) {
		ResultSet resul = db.getSelect(UsersAccess.getSql(uId, aId));
	...
	public static String getSql(int id_users, int id_access) {
		...
		if (id_users > 0 || id_access > 0)
			sql += " WHERE ";
		if (id_users > 0)
			sql += table + ".id_users = " + id_users;
		if (id_users > 0 && id_access > 0)
			sql += " AND ";
		if (id_access > 0)
			sql += table + ".id_access = " + id_access;
		return sql;
	}

	public static void save(int id_users, int id_access, MySQL db) {
		String table = getTable();
		...
		if (isNew == null) {
			...
			db.update(table, fields, new UsersAccess(id_users, id_access).getValues(),
					new String[] { table + ".id_users =" + isNew.getId_users() });
		}
	}

	public static boolean delete(int id_users, int id_access, MySQL db) {
		String table = getTable();
		if (id_users > 0) {
			db.delete(table,
					new String[] { table + ".id_users =" + id_users + " AND " + table + ".id_access =" + id_access });
		}
		UsersAccess isNew = UsersAccess.get(id_users, id_access, db);
