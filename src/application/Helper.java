...
public final class Helper {
	// this variables needed to check if operation or place or ip changed from last
	// call of
	// formatOutput method
	 ...
	 *
	 * @throws IllegalAccessException will be thrown if instance of class will be
	 *                                created
	 */
	...
	/**
	 * Method will create formatted string for outputting to "Журнал подій", there
	 * are two types of string can be created: 1. If operation or place or ip is
	 * null or changed string will look like "Operation на (вагах/сервері): ip
	 * \n\tSectionType: name [date] status" 2. If operation and place and ip are the
	 * same as the last time string will look like " SectionType: name [date]
	 * status"
	 *
	 * @param operation       enum which contains operations which can be done with
	 *                        objects
	 * @param place           enum which contains places where operation can happen
	 * @param ip              string which contains ip address of place which called
	 *                        operation
	 * @param sectionType     enum which contains section at which operations can
	 *                        happen
	 * @param name            string which contains name of object on which
	 ...
	 * @throws NoSuchElementException will be thrown if place is null or has value
	 *                                that is not in switch
	 * @throws NullPointerException   will be thrown if operation is null
	 */
	public static String formatOutput(Operation operation, PlaceType place, String ip, SectionType sectionType,
			String name, String dateTime, OperationStatus operationStatus)
			throws NoSuchElementException, NullPointerException {
		if (operation == null)
			throw new NullPointerException("Operation can not be null");

		boolean operationOrPlaceChanged = lastExecutedOperation == null || !lastExecutedOperation.equals(operation)
				|| lastPlaceType == null || !lastPlaceType.equals(place) || lastIp == null || !lastIp.equals(ip);

		lastExecutedOperation = operation;
		lastPlaceType = place;
		lastIp = ip;

		String baseString = String.format("\t%s: %s [%s]-%s", capitalizeFirstLetter(sectionType.getName()), name,
				dateTime, operationStatus.getName());

		if (operationOrPlaceChanged) {
			return String.format("%s на %s:%s" + System.lineSeparator(), capitalizeFirstLetter(operation.getName()), // with
																														// first
																														// letter
																														// capitalized
					PlaceType.SCALE.equals(place) ? "вагах" : "сервері", ip) + baseString;
		} else {
			return baseString;
		}
	}
	...
	public static Optional<List<Object>> getData(MySQL db, int limit, int offset, ObjectType type) {
		SqlQueryBuilder queryBuilder = new SqlQueryBuilder(db, type.getTableName());
		List<Object> list = null;
		try {
			ResultSet resultSet = queryBuilder.select("*").from(type.getTableName())
					.orderBy(type.getTableName(), type.getOrderByColumn()).limit(limit).offset(offset).execute();
			...
			case TEMPLATES_CODES:
				...
				break;
			case SETTINGS:
				NodeTree node = SidebarCtrl.getSelectedItem().getValue();
				if (node.getLevel() == 2)
					list = MainCtrl.getSettings();
				else {
					List<Scales> res = Converter.fromResultSetToScalesList(resultSet);
					list = res.size() > 0 ? res.get(0).getColumsInfo() : null;
				}
				break;
			default:
	...
	public static Optional<List<Goods>> getGoods(MySQL db, int limit, int offset, ObjectType type) {
		SqlQueryBuilder queryBuilder = new SqlQueryBuilder(db, type.getTableName());
		ResultSet resultSet = queryBuilder.select("*").from(type.getTableName())
				.orderBy(type.getTableName(), type.getOrderByColumn()).limit(limit).offset(offset).execute();
		try {
