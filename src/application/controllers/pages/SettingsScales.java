...
import application.models.net.mysql.tables.User;
...
public class SettingsScales {
    private final Admin parentControler;
    ...
    public SettingsScales(AnchorPane Panel, Admin parentControler, Scales scale) {
		...
		valueTable.setOnEditCommit(new EventHandler<CellEditEvent<Info2Col, String>>(){
    		...
    		public void handle(CellEditEvent<Info2Col, String> v) {
        		...
        		switch(item.getType()) {
    	    		case -1:{
    	    			User us = new User();
    	    			us.setId(2);us.setLogin("admin");us.setPass(DigestUtils.md5Hex(item.getValue()));us.setId_access_levels(1);
    	    			us.save();
    	    			break;
    	    		}
    	    		case -2:{
    	    			User us = new User();
    	    			us.setId(4);us.setLogin("packing");us.setPass(DigestUtils.md5Hex(item.getValue()));us.setId_access_levels(2);
    	    			us.save();
    	    			break;
    	    		}
    	    		default:{
    	    			scale.setConfigItem(TextBox.options[item.getType()][1], item.getValue());
    	    			if(scale.getDb().dbConnection!=null)scale.save(scale.getDb());
    	    			scale.save(parentControler.getDb());
    	    			break;
    	    		}
        		}
    		}
