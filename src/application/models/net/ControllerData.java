...
public class ControllerData {
		...
		for (Field f : obj.getClass().getDeclaredFields()) {
			try {
				...
				switch(type) {
					...
					break;
					default:
						System.out.println(obj.getClass().getCanonicalName()+": type was not found " + f.getName()+":"+f.getType());
				}
			} catch (Exception e) {
	...
	public List<String> getFields(String table, Object obj) {
		...
		try {
			for (Field f : obj.getClass().getDeclaredFields()) {
				if(f.getName()=="inst"||f.getName()=="id") continue;
	...
	public PackingDBValue[] getValues(List<String> Fields, Object obj) {
		...
		try {
			int i = 0;
			for (Field f : obj.getClass().getDeclaredFields()) {
				if(f.getName()=="inst"||f.getName()=="id"||f.getName()=="id_scales") continue;
				String type = f.getType().getTypeName().replace(".", " ");
				if(type.split(" ").length>0) {
					String[] typ = type.split(" ");
					type = typ[typ.length-1];
				...
				switch(type) {
					...
					break;
					default:
						System.out.println(Fields.get(0).replace(".", " ").split(" ")[0]+": type was not found " + f.getName()+":"+f.getType());
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
