...
public class Goods {
	...
	public Goods(ResultSet res,int[] id) {
		super();
		if(this.getId_barcodes()==0)this.setId_barcodes(id[0]);
		if(this.getId_templates()==0)this.setId_templates(id[1]);
		try {
	...
	public static String getSqlCount(int idScale, String table) {
		...
		if(idScale > 0) sql += " WHERE ";
		if(idScale > 0) sql +=table+".id_scales = "+idScale;
		return sql;
	}
	...
	public static String getSql(int number, int pre_code, String name, int sId, int tId) {
		String table = getTable();
		String sql = "SELECT * FROM "+table;
		if(number > 0||pre_code > 0||sId > 0||tId > 0||name.length() > 0) sql += " WHERE ";
		if(number > 0) sql += table+".code = \""+number+"\" ";
		if(number > 0&&(pre_code > 0||sId > 0||tId > 0||name.length() > 0)) sql += " AND ";
		if(pre_code > 0) sql += table+".pre_code = \""+pre_code+"\" ";
		if(pre_code > 0&&(sId > 0||tId > 0||name.length() > 0)) sql += " AND ";
		if(sId > 0) sql +=table+".id_sections = "+sId;
		if(sId > 0&&(tId > 0||name.length() > 0)) sql += " AND ";
		if(tId > 0) sql+= table+".id_templates = "+tId;
		if(tId > 0&&name.length() > 0) sql += " AND ";
	...
	public int save(MySQL db) {
		if(id!=-1) {
			...
			if(isNew==null){
				...
				db.update(table, getFields().toArray(new String[0]), getValues(), new String[]{table+".pre_code ="+pre_code});
			}
	...
	public void updatePre_code(int pre_code, MySQL db) {
		String table = getTable();
		Goods isNew = Goods.get(0,pre_code,"",0,0,db);
		if(isNew==null)db.update(table,new String[]{(table+".pre_code")}, PackingDBValue.get(new String[] {"int"}, new Object[] {pre_code}), new String[]{table+".pre_code = "+this.pre_code});
		if(Goods.get(0,pre_code,"",0,0,db)!=null)this.setPre_code(pre_code);
	}
