...
public class BotsTelegram {
	...
	public static ObservableList<BotsTelegram> getList(int id, String name, String phone, String chat_id, int included, MySQL db) {
	...
	public static String getSql(int id, String name, String address, String api_token, int included) {
		...
		if(id > 0 || name.length() > 0 || address.length() > 0 || api_token.length() > 0||included > -1) sql+= " WHERE ";
		if(id > 0) sql+= table+".id = "+id;
		if(id > 0&&(name.length() > 0||address.length() > 0||api_token.length() > 0||included > -1)) sql += " AND ";
		if(name.length() > 0) sql+= table+".name = '"+name+"'";
		if(name.length() > 0&&(address.length() > 0||api_token.length() > 0||included > -1)) sql += " AND ";
		if(address.length() > 0) sql+= table+".address = '"+address+"'";
		if(address.length() > 0&&(api_token.length() > 0||included > -1)) sql += " AND ";
		if(api_token.length() > 0) sql+= table+".api_token = '"+api_token+"'";
		if(api_token.length() > 0&&(included > -1)) sql += " AND ";
		if(included > -1) sql+= table+".included = "+included;
		return sql;
	}
	public static String getSql(String name) {
		String table = getTable();
		String sql = "SELECT * FROM "+table;
		if(name.length() > 0) sql+= " WHERE ";
	...
	public int save(MySQL db) {
		...
		if(isNew==null) {
			...
			db.update(table,me.getFields().toArray(new String[0]), me.getValues(), new String[]{table+".name = '"+me.getName()+"'"});
		}
	...
	public void updateName(String name,MySQL db) {
		...
		if(isNew==null)db.update(table,new String[]{(table+".name")}, PackingDBValue.get(new String[] {"String"}, new Object[] {name}), new String[]{table+".name = '"+me.getName()+"'"});
		if(BotsTelegram.get(0,name,"","",-1,db)!=null)this.setName(name);
	}
