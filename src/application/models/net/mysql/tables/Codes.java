...
public class Codes {
	...
	public Codes(ResultSet res) {
		...
		if(this.name.length()==0)this.name = this.prefix + "/" + this.code + "/" + this.unit;
	}
	...
	public void updateId(int id,MySQL db) {
		...
		if(isNew==null)db.update(table,new String[]{(table+".id")}, PackingDBValue.get(new String[] {"int"}, new Object[] {id}), new String[]{table+".id = "+getId()});
		if(Codes.get(id,"",db)!=null)this.setId(id);
	}
	public void updateName(String name,MySQL db) {
		String table = getTable();
		Codes isNew = Codes.get(0,name,db);
		if(isNew==null)db.update(table,new String[]{(table+".name")}, PackingDBValue.get(new String[] {"String"}, new Object[] {name}), new String[]{table+".name = '"+getName()+"'"});
		if(Codes.get(0,name,db)!=null)this.setName(name);
	}
	/*public static void getResponse(Scales scale, MySQL dbServer) {
		Codes me = new Codes();
		MySQL dbScale = scale.getDb();
		ResultSet resul = dbScale.getSelect(Codes.getSql(0,0,"",dbScale,false));
		String[] table = getTable();
    	dbServer.insertAll(table[0], me.getFields().toArray(new String[0]), me.getValues(),resul);
		dbServer.insertAll(table[1], Codes.getScales(), Codes.getScalesValue(me.getId(),me.getId_scales(),me.getNumber()),resul);
	}*/
