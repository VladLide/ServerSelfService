...
public class Sections {
	...
	public static String getSql(int id, int id_up, int level, String name, boolean img) {
		...
		if(id > 0||id_up>=0||level>0||name.length()>0) sql+= " WHERE ";
		if(id > 0) sql+= table+".id = "+id;
		if(id > 0&&(id_up>=0||level>0||name.length()>0)) sql+= " AND ";
		if(id_up >= 0) sql+= table+".id_up = "+id_up;
		if(id_up>=0&&(level>0||name.length()>0)) sql+= " AND ";
		if(level > 0) sql+= table+".level = "+level;
		if(level > 0&&name.length()>0) sql+= " AND ";
	...
	public void updateId(int id, MySQL db) {
		...
		if(Sections.get(id,-1,0,"",false,db)==null) {
			db.update(table,new String[]{(table+".id")}, PackingDBValue.get(new String[] {"int"}, new Object[] {id}), new String[]{table+".id = "+this.id});
	...
	public void updateName(String name,MySQL db) {
		...
		if(Sections.get(0,-1,0,name,false,db)!=null)this.setName(name);
	}
