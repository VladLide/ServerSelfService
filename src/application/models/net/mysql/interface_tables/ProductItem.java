...
public class ProductItem {
	...
	public ProductItem(int i, ResultSet res, MySQL db) {
		...
		for (Field f : getClass().getDeclaredFields()) {
			if(f.getName()=="inst"||f.getName()=="num") continue;
			...
			try {
				...
				case "Sections":{
					try {
						f.set(this, new Sections(res.getInt(res.findColumn("sId")), res.getString(res.findColumn("sName"))));
					}catch (Exception e) {
				...
				case "Templates":{
					try {
						f.set(this, new Templates(res.getInt(res.findColumn("tId")),res.getString(res.findColumn("tName")).toUpperCase(),res.getBlob(res.findColumn("tData")),
							res.getBlob(res.findColumn("templatesImg_data")),res.getBlob(res.findColumn("background_data"))));
					}catch (Exception e) {
				...
				case "Codes":{
					try {
						f.set(this, new Codes(res.getInt(res.findColumn("cId")),res.getString(res.findColumn("cname")),res.getString(res.findColumn("cmaska")),res.getString(res.findColumn("cmask"))));
					}catch (Exception e) {
						f.set(this,new Codes());
					}
				break;
	...
	public static String getSqlHeader(int type) {
		String sql = "*";
		switch (type) {
		case 1: sql = "id, id_up, level, name, description, number_s, number_po";
			break;
		case 2: sql = "id, id_up, level, name, description, number_s, number_po";
			break;
		case 3: sql = "id, id_up, level, name, description, number_s, number_po";
		break;
		default:
			break;
		}
	...
	public static String getSql(int sId, String name, int number, int pre_code, int union, int limit, boolean like) {
		...
		if(sId > 0||name.length() > 0||number > 0||pre_code > 0) sql += " WHERE ";
		if(number > 0) sql += table+".code = "+((like)?("LIKE \""+number+"%\""):("\""+number+"\" "));
		if(number > 0&&(sId > 0||name.length() > 0)) sql += " AND ";
		if(sId > 0) sql +=table+".id_scales = "+sId;
		if(sId > 0&&name.length() > 0) sql += " AND ";
		if(name.length() > 0) sql += table+".name = "+((like)?("LIKE \"%"+name+"%\""):("\""+name+"\""));
		sql+=" ORDER BY "+table+".code ";
		if(limit>0)sql += " LIMIT "+limit+" ";
		return sql;
	}
	public static ProductItem get(int sId, String name, int number, int pre_code, int union, int limit, boolean like, MySQL db) {
	...
	public static ObservableList<ProductItem> getList(int sId, String name, int number, int pre_code, int union, int limit, boolean like, MySQL db) {
    	ResultSet resul = db.getSelect(ProductItem.getSql(sId, name, number, pre_code, union, limit, like));
	...
	public static ObservableList<Object> getListObj(int sId, String name, int number, int pre_code, int union, int limit, boolean like, MySQL db) {
    	ResultSet resul = db.getSelect(ProductItem.getSql(sId, name, number, pre_code, union, limit, like));
	...
	public int save(int idScale, int idSection, int idBarcode, MySQL db) {
		...
		for (Field f : me.getClass().getDeclaredFields()) {
			if(f.getName()=="inst"||f.getName()=="sectionsName"||f.getName()=="template"||f.getName()=="barcode") continue;
			try {
				String type = f.getType().getTypeName().replace(".", " ");
				if(type.split(" ").length>0) {
					String[] typ = type.split(" ");
