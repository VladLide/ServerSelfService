...
public class UsersTelegram {
	...
	public static String getSql(int id, String name, String phone, String chat_id) {
		...
		if (id > 0 || name.length() > 0 || phone.length() > 0 || chat_id.length() > 0)
			sql += " WHERE ";
		if (id > 0)
			sql += table + ".id = " + id;
		if (id > 0 && (name.length() > 0 || phone.length() > 0 || chat_id.length() > 0))
		...
		if (name.length() > 0)
			sql += table + ".name = '" + name + "'";
		if (name.length() > 0 && (phone.length() > 0 || chat_id.length() > 0))
			sql += " AND ";
		if (phone.length() > 0)
			sql += table + ".phone = '" + phone + "'";
		if (phone.length() > 0 && (chat_id.length() > 0))
			sql += " AND ";
		if (chat_id.length() > 0)
			sql += table + ".chat_id = '" + chat_id + "'";
		return sql;
	}
	...
	public int save(MySQL db) {
		UsersTelegram me = this;// init();
		String table = getTable();
		if (me.getChat_id().length() < 1)
			me.setChat_id(me.getId() + "");
		...
		if (isNew == null) {
			...
			db.update(table, me.getFields().toArray(new String[0]), me.getValues(),
					new String[] { table + ".chat_id =" + me.getChat_id() });
		}
	...
	public void updateChat_id(String chat_id, MySQL db) {
		...
		if (isNew == null)
			db.update(table, new String[] { (table + ".chat_id") },
					PackingDBValue.get(new String[] { "String" }, new Object[] { chat_id }),
					new String[] { table + ".chat_id = '" + me.getChat_id() + "'" });
		if (UsersTelegram.get(0, "", "", chat_id, db) != null)
			this.setName(name);
	}
