...
public class Stocks {
		...
		for (Field f : me.getClass().getDeclaredFields()) {
			try {
				...
				case "String":
					...
					break;
				case "boolean":
					values[i++] = new PackingDBValue(f.getName(), "BL", (Object) f.get(me));
					break;
				default:
	...
	public static String getSql(int pre_code) {
		...
		if (pre_code > 0)
			sql += " where " + table + ".code_goods = " + pre_code;
		return sql;
	}
	...
	public boolean save(MySQL db) {
		...
		if (me.getCode_goods() != 0) {
			...
			if (isNew == null) {
				...
				db.update(table[0], me.getFields().toArray(new String[0]), me.getValues(),
						new String[] { table[0] + ".code_goods =" + me.getCode_goods() });
			}
			;
		}
	...
	public void updateCode(int code, MySQL db) {
		...
		if (isNew == null)
			db.update(table, new String[] { (table + ".code_goods") },
					PackingDBValue.get(new String[] { "int" }, new Object[] { code }),
					new String[] { table + ".code_goods = " + me.getCode_goods() });
		if (Stocks.get(code, db) != null)
			this.setCode_goods(code);
	}
