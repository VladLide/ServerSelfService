...
public class Users {
	...
	/*
	 * private Map<Integer, Access> access = new HashMap<Integer, Access>(); private
	...
	public List<String> getFields() {
		...
		try {
			for (Field f : me.getClass().getDeclaredFields()) {
				if (f.getName() == "num" || f.getName() == "id" || f.getName() == "accessName"
						|| f.getName() == "access" || f.getName() == "move")
					continue;
	...
	public Users(int i, ResultSet res) {
		...
		for (Field f : getClass().getDeclaredFields()) {
			try {
				if (f.getName() == "num" || f.getName() == "accessName" || f.getName() == "access"
						|| f.getName() == "move")
					continue;
				String type = f.getType().getTypeName().replace(".", " ");
				if (type.split(" ").length > 0) {
	...
	public static Users get(int id, String code, String login, String name, String pass, MySQL db) {
		Users res = null;
	...
	public static ObservableList<Users> getList(int id, String code, String login, String name, MySQL db) {
	...
	public static ObservableList<Users> getList(String login, String name, MySQL db) {
	...
	public static String getSql(int id, String code, String login, String name, String pass) {
		String table = getTable();
		String sql = "SELECT * FROM " + table;
		if (id > 0 || code.length() > 0 || login.length() > 0 || name.length() > 0 || pass.length() > 0)
			sql += " WHERE ";
		if (id > 0)
			sql += table + ".id = " + id;
		if (id > 0 && (code.length() > 0 || login.length() > 0 || name.length() > 0 || pass.length() > 0))
		...
		if (code.length() > 0)
			sql += table + ".code = '" + code + "'";
		if (code.length() > 0 && (login.length() > 0 || name.length() > 0 || pass.length() > 0))
			sql += " AND ";
		if (login.length() > 0)
			sql += table + ".login = '" + login + "'";
	...
	public void updateCode(String newCode, MySQL db) {
		String code = newCode + "";
		Users me = this;
		String table = getTable();
		Users isNew = Users.get(0, me.getCode(), "", "", "", db);
		if (isNew != null)
			db.update(table, new String[] { (table + ".code") },
					PackingDBValue.get(new String[] { "String" }, new Object[] { code }),
					new String[] { table + ".code = '" + me.getCode() + "'" });
		if (Users.get(0, code, "", "", "", db) != null)
			this.setCode(code);
	}
	...
	/*
	 * public Map<Integer, Access> getAccess() { return access; } public Access
	 * getItemAccess(int key) { return access.get(key); } public void
	 * setAccess(Map<Integer, Access> access) { this.access = access; } public void
	 * setItemAccess(int key, Access value) { this.access.put(key, value); }
	 */
	...
	/*
	 * public ObservableList<Move> getMove() { return move; } public void
	 * setMove(ObservableList<Move> move) { this.move = move; } public Move
	 * getItemMove(int key) { return move.get(key); } public void setItemMove(Move
	 * value) { this.move.add(value); }
	 */
