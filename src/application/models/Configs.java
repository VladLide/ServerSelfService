...
public class Configs {
    ...
    public void readConfigFile(String name) {
    	...
    	try {
			...
			String[] nextLine = {};
			if(me.file!=null){
				if(me.file.length()>0)nextLine = me.file.split(System.getProperty("line.separator"));
			}
	...
	public Map<String, String> writeFile(String name) {
    	Configs me = getInstance();
		me.file = "";
		me.config.forEach((key,value)->{
			me.file +=key+"="+value+System.getProperty("line.separator");
	...
	public static void setItem(String key, Double value, Integer singShare) {
		...
		getInstance().config.put(key, String.format("%."+singShare+"f",(double)Math.round(value * dil) / dil).replace(",", "."));
	}
